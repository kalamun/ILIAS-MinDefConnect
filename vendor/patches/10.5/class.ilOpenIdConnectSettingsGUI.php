 ...
 *
 * PATCH: Added "Use UserInfo Endpoint" checkbox to the settings form.
 *
 * Only the two methods that build and process the main settings form have
 * been changed: initSettingsForm() and saveSettings(). All other methods
 * are left exactly as they were in the original file so you can apply this
 * as a targeted diff rather than replacing the whole file.
 *
 * SEARCH FOR  "// --- PATCH ---"  to find every changed line.
 */
...
class ilOpenIdConnectSettingsGUI
{
    ...
    // -----------------------------------------------------------------------
    // Settings form — PATCHED
    // -----------------------------------------------------------------------

    /**
     * Build the main OpenID Connect settings form.
     *
     * PATCH: A new "Use UserInfo Endpoint" checkbox is inserted directly after
     * the "Active" checkbox so it is visible at the top of the form.
     */
     ...
     *
     * PATCH: reads the new 'use_userinfo_endpoint' checkbox and calls
     * $this->settings->setUseUserinfoEndpoint().
     */
