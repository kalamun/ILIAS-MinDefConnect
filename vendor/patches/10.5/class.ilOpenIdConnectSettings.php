...
class ilOpenIdConnectSettings
{
    ...
    // -----------------------------------------------------------------------
    // NEW: UserInfo endpoint getter / setter
    // -----------------------------------------------------------------------

    /**
     * Returns true when ILIAS should call the IdP's UserInfo endpoint after
     * token exchange and merge the returned claims into the ID-token claims.
     */
    public function getUseUserinfoEndpoint(): bool
    {
        return $this->use_userinfo_endpoint;
    }

    /**
     * Enable or disable calling the UserInfo endpoint during authentication.
     *
     * Set to true for IdPs that return additional profile claims (email,
     * given_name, …) only via the UserInfo endpoint rather than embedding
     * them directly in the ID token.
     */
